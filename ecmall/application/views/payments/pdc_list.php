<?php card_start('Post-Dated Cheques (Supplier Payments)', true); ?>

<div class="mb-6 flex justify-between items-center">
    <p class="text-gray-600">Manage post-dated cheques issued to suppliers</p>
    <a href="<?php echo base_url('payments'); ?>" class="btn btn-secondary">
                ...
                <tbody>
                    <?php foreach ($due_clearance as $pdc): ?>
                        <tr class="border-b">
                            ...
                            <td class="px-4 py-2"><strong><?php echo htmlspecialchars($pdc->supplier_name ?? 'N/A'); ?></strong></td>
...
<div class="overflow-x-auto">
    <?php table_start(['Date', 'Cheque Date', 'Supplier', 'Cheque No', 'Bank', 'Amount', 'Status']); ?>
        ...
        <?php else: ?>
            <?php foreach ($pending_pdcs as $pdc): ?>
                <tr class="<?php echo ($pdc->chqdate <= date('Y-m-d')) ? 'bg-warning-50' : ''; ?>">
                    <td><?php echo format_date($pdc->tdate); ?></td>
                    <td class="font-semibold"><?php echo format_date($pdc->chqdate); ?></td>
                    <td><strong><?php echo htmlspecialchars($pdc->supplier_name ?? 'N/A'); ?></strong></td>
...
function clearPDC(pdcId, chequeNo) {
    Swal.fire({
        title: 'Clear PDC?',
        html: '<p>Cheque: <strong>' + chequeNo + '</strong></p><div class="mt-4"><label class="block text-sm font-medium text-gray-700 mb-2">Clearance Date:</label><input type="date" id="clearance_date" value="<?php echo date('Y-m-d'); ?>" class="w-full px-3 py-2 border rounded"></div>',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#10B981',
        confirmButtonText: 'Yes, clear it!',
        preConfirm: () => document.getElementById('clearance_date').value
    }).then((result) => {
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '<?php echo base_url('payments/clear_pdc/'); ?>' + pdcId;
